...
`Python` `Java` `C++` `JavaScript` `MATLAB` `SQL` `Arduino C++`

### AI / Machine Learning & Computer Vision

`PyTorch` `YOLO` `DeepLabV3` `OpenCV` `NumPy` `Pandas` `Matplotlib`

`Machine Learning` `Deep Learning` `Computer Vision` `NLP`

### Algorithms & Data Structures

`AVL Trees` `Tries` `Segment Trees` `Heaps` `Hash Maps` `Graphs`

`BFS` `DFS` `Dijkstra` `DSU` `Dynamic Programming` `Linear Programming`

`Sorting & Searching` `Linked Lists` `Stacks & Queues`

### Software Engineering

`Spring Boot` `Spring MVC` `REST APIs` `HTTP/HttpClient`

`JSON/Jackson` `Node.js` `Chart.js` `HTML/CSS` `MySQL`

### Robotics & Embedded Systems

`Arduino` `ESP32` `Servo Control` `FSR/Tactile Sensors`
...
**Python • PyTorch • Torchvision • DeepLabV3 • NumPy • OpenCV**

A deep-learning computer vision pipeline for semantic segmentation of Martian terrain.
...
## Financial Expense Optimization System
...
## Collective Motion with Informed Leaders

**Python • NumPy • Pandas • Matplotlib • Scientific Computing • Swarm Intelligence**
...
**MATLAB • Simulink • Graph Theory • Dijkstra • Linear Programming**
